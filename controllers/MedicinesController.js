...
exports.createMedicine = async (req, res) => {
  ...
  let validate = validateBody(
    await Medicines(DB, DataTypes).describe(),
    req.body
  );
  ...
  await Medicines(DB, DataTypes)
    .create({ name, treatment, unit, id_visit, id_pet, status: 1 })
    .then(() => {
      res.status(200);
      res.json({ msg: "OK" });
    })
...
module.exports.getMedicines = async (req, res) => {
  await Medicines(DB, DataTypes)
    .findAll({ where: { id_pet: req.params.idPet, status: { [Op.ne]: 0 } } })
    .then((data) => {
      res.status(200);
      res.json(data);
    })
